...
# @Last modified time: 2018-12-24T02:47:06-06:00
...
class TermViewSet(viewsets.ModelViewSet):
    ...
    def get_pages(self, request):
		...
		if namespace is not None and ontology_id is not None:
			if namespace is 'bp':
				name_query = "[u'biological_process']"
				
			objs = Term.objects.filter(ontology__id=ontology_id, namespace=name_query).order_by('termid').count()
